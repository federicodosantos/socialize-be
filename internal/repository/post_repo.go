...
func (r *PostRepo) CreatePost(ctx context.Context, post *model.Post) (int64, error) {
	...
	insertPostQuery := fmt.Sprintf(`INSERT INTO posts(id, user_id, content, created_at, updated_at)
				VALUES('%s', '%s', '%s', '%s', '%s')`, post.ID, post.UserID, post.Content, createdAtStr, updatedAtStr)
...
func (r *PostRepo) GetPostByID(ctx context.Context, postId string) (*model.Post, error) {
	var post model.Post

	query := fmt.Sprintf("SELECT * FROM posts WHERE id = '%s'", postId)
